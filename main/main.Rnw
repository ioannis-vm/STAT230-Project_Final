...
We heavily encourage a visit to our repository.
...
The variables \texttt{Overall.Qual} and \texttt{Overall.Cond} are a rating in the scale from 1 to 10 of the overall quality and condition of the considered house, respectively.
...
The following code computes the t-statistic for the intercept and the coefficient using the five corrections covered in the course material.
...
We observe that all five EHW corrections result in increased standard errors for the regression coefficients, which is in line with our expectation.
