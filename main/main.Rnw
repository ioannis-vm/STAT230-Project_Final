...
While this is not always the case, here both directions result in the same model.
The following figure shows the AIC obtained in each step starting from the full regression, as well as starting from the mean model.
